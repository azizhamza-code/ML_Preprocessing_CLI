...
class BaseLitModel(pl.LightningModule):
    """
    Generic Pythorch-Lighning class that be initialized with a pytorch module
    ...
    def __init__(self,model, args:argparse.Namespace = None):
        ...
        self.class_optimizer = getattr(torch.optim , optimizer)

        self.lr = self.args.get("lr",LR)

        loss = self.args.get("loss",LOSS)
        if loss not in ("ctc","transformer"):
            self.loss_fn = getattr(torch.nn.functionnal , loss)
        ...
        self.vsl_acc = Accuracy()


        @staticmethod
        def add_to_argparse(parser):

            parser.add_argument("--optimiser",type = str , default = OPTIMIZER ,help ="optimizer class from torch.optim")
